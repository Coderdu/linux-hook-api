...
void print_elf_header(Elf64_Ehdr *e_hdr_ptr, FILE *file)
{
	...
	fprintf(f, "(Entry point virtual address)e_entry: 0x%02llx\r\n",e_hdr_ptr->e_entry);
	fprintf(f, "(Program header table file offset)e_phoff: 0x%02llx\r\n",e_hdr_ptr->e_phoff);
	fprintf(f, "(Section header table file offset)e_shoff: 0x%02llx\r\n",e_hdr_ptr->e_shoff);
...
void print_program_header(Elf64_Phdr *p_hdr_ptr, FILE *file)
{
	...
	fprintf(f, "(Segment file offset)p_offset: 0x%02llx\n",	p_hdr_ptr->p_offset);
	fprintf(f, "(Segment virtual address)p_vaddr: 0x%02llx\n",p_hdr_ptr->p_vaddr);
	fprintf(f, "(Segment physical address)p_paddr: 0x%02llx\n",p_hdr_ptr->p_paddr);
	fprintf(f, "(Segment size in file)p_filesz: 0x%02llx\n",p_hdr_ptr->p_filesz);
	fprintf(f, "(Segment size in memory)p_memsz: 0x%02llx\n",p_hdr_ptr->p_memsz);
	fprintf(f, "(Segment alignment)p_align: 0x%02llx\n",p_hdr_ptr->p_align);
...
void print_section_header(Elf64_Shdr *s_hdr_ptr, char * sec_name_str_table, Elf64_Xword size, FILE *file)
{
	...
	fprintf(f, "(Section flags)sh_flags: 0x%02llx\n",s_hdr_ptr->sh_flags);
	fprintf(f, "(Section virtual addr at execution)sh_addr: 0x%02llx\n",s_hdr_ptr->sh_addr);
	fprintf(f, "(Section file offset)sh_offset: 0x%02llx\n",s_hdr_ptr->sh_offset);
	fprintf(f, "(Section size in bytes)sh_size: 0x%02llx\n",s_hdr_ptr->sh_size);
	fprintf(f, "(Link to another section)sh_link: 0x%02x\n",s_hdr_ptr->sh_link);
	fprintf(f, "(Additional section information)sh_info: 0x%02x\n",s_hdr_ptr->sh_info);
	fprintf(f, "(Section alignment)sh_addralign: 0x%02llx\n",s_hdr_ptr->sh_addralign);
	fprintf(f, "(Entry size if section holds table)sh_entsize: 0x%02llx\n",s_hdr_ptr->sh_entsize);
...
void print_sym(Elf64_Sym *sym_ptr, char * SymNamStrTable, FILE *file)
{
	...
	fprintf(f, "Symbol_value: 0x%02llx\n", sym_ptr->st_value);
	fprintf(f, "Symbol_size: 0x%02llx\n", sym_ptr->st_size);
