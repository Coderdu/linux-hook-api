...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	fs.open("linuxlib\\libc-2.5.so", std::ios::in | std::ios::binary);
	...
	if((file = fopen("result\\elf_header.txt", "w")) == NULL)
	{
	...
	if((file = fopen("result\\program_header.txt", "w")) == NULL)
	{
	...
	if((file = fopen("result\\section_header.txt", "w")) == NULL)
	{
	...
	if((file = fopen("result\\sym_table.txt", "w")) == NULL)
	{
		printf("File cannot be opened: section_header.txt\n");
		return -1;
	}
